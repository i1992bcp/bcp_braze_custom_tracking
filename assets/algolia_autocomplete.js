(function(algolia) {
  ...
  var client = algoliasearch( algolia.config.app_id, algolia.config.search_api_key );
  ...
  function createMultiQuerySource() {
    ...
    return function multiQuerySource(singleIndex, params, extendHits = null) {
      return function search(query, cb) {
        const index = queue.push({ indexName: singleIndex.indexName, query, params }) - 1;
        requestSearch(index)
          .then((res) => {
            var extendedHits = [];
            if (extendHits) {
  ...
  function autocompleteSection(config, section) {
    ...
    var templates = {
      ...
      suggestion: function displaySuggestion(hit) {
        if(hit._index === "prod_products_query_suggestions"){
  ...
  var config = {
    ...
    products_query_suggestions: {
      hitsPerPage: 4,
      index: index('products_query_suggestions'),
      templates:{
        empty: `<div></div>`,
        hit: algolia.compileTemplate('autocomplete_suggestion'),
      }
    },
  };
  ...
  var autocompleteInstance = autocomplete(
    ...
    [
      algolia.config.index_collections && autocompleteSection(config, 'collections'),
      ...
      algolia.config.index_products && autocompleteSection(config,'products_query_suggestions'),
    ].filter(function(x) {
      return Boolean(x);
    }));
